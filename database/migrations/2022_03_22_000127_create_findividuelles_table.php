...
class CreateFindividuellesTable extends Migration
{
    ...
    public function up()
    {
        Schema::create($this->tableName, function (Blueprint $table) {
            ...
            $table->index(["projets_id"], 'fk_findividuelles_projets1_idx');

            $table->index(["programmes_id"], 'fk_findividuelles_programmes1_idx');
            $table->softDeletes();
            ...
            $table->foreign('projets_id', 'fk_findividuelles_projets1_idx')
                ->references('id')->on('projets')
                ->onDelete('no action')
                ->onUpdate('no action');

            $table->foreign('programmes_id', 'fk_findividuelles_programmes1_idx')
