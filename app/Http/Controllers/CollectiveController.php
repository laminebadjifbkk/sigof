...
class CollectiveController extends Controller
{
    ...
    public function store(Request $request)
    {
        $this->validate($request, [
            ...
            "telephone_responsable" => ["required", "string", "size:9", Rule::unique('collectives')->where(function ($query) {
                return $query->whereNull('deleted_at');
            })],
    ...
    public function update(Request $request, Collective $collective)
    {
        ...
        if (! Auth::user()->hasRole(['super-admin', 'admin', 'Ingenieur'])) {
    ...
    public function liste(Request $request, $id)
    {
        ...
        $title = 'Liste de demande collective ' . $collective->name;
        ...
        $html = View::make('collectives.liste', compact(
            'collective',
            'title',
        ))->render();
    ...
    public function generateReport(Request $request)
    {
        ...
        $collectives = Collective::join('users', 'users.id', 'collectives.users_id')
            ->select('collectives.*')
