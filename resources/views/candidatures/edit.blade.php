...
@section('content')
    <div class="dash-topbar">
        <div>
            ...
            <p class="muted-sub">{{ $candidature->user->firstname }} {{ $candidature->user->name }} — Candidature
                #{{ str_pad($candidature->id, 6, '0', STR_PAD_LEFT) }}</p>
        </div>
    ...
    <div class="panel">
        ...
        <form action="{{ route('candidatures.update', $candidature) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="field-row">
                <div class="field">
                    ...
                    <select name="diplome">
                        ...
                        <option value="certification" @selected(old('diplome', $candidature->diplome) == 'certification')>Certification linguistique reconnue
                        </option>
                    </select>
                    @error('diplome')
                        <span class="field-error">{{ $message }}</span>
            ...
            @foreach ($currentFiles as $field => $info)
                <div class="field" style="margin-bottom:16px;">
                    ...
                    @else
                        <p style="font-size:12.5px; color:var(--gray-500,#888); margin-bottom:6px;">Aucun fichier
                            actuellement.</p>
                    @endif
