...
<body>
    ...
    <h4 style="text-align: center;">FICHE DE SYNTHESE DU DOSSIER D'AGREMENT REGION DE
        {{ strtoupper($region?->nom) . ' : ' . $operateurs->count() }}
    </h4>
    <?php $i = 1; ?>
                ...
                <thead>
                    <tr class="heading">
                        <td colspan="5">{{ $i++ }} / {{ $operateurs->count() }} - DOSSIER D'AGREMENT N° :
                            <span style="color: red">{{ $operateur?->numero_dossier }}</span>
                        </td>
                        <td colspan="2"></td>
