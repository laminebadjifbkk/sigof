                    ...
                    <div class="card-body">
                                                ...
                                                <div class="d-flex align-items-center mb-2">
                                                    ...
                                                    <div class="ps-2">
                                                        <h6 class="mb-0" style="font-size: 0.9rem;">
                                                            {{ number_format(count($cal?->operateurs), 0, '', ' ') }}</h6>
                        ...
                        @can('export-operateur')
                            {{-- <div class="d-flex justify-content-end align-items-center mb-3">
                                <a href="{{ route('export.operateurs.all', ['commissionagrement' => $cal?->id]) }}"
                                    class="btn btn-success btn-sm me-2" title="Exporter les opérateurs + scans">
                                    <i class="bi bi-file-earmark-excel"></i> Exporter opérateurs + scans
                                </a>
                            </div> --}}
                            {{-- <div class="d-flex justify-content-end align-items-center mb-3"> --}}
                            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
                                <span class="d-flex align-items-baseline">
                                    <a href="{{ route('export.ficheSynthese.all', ['commissionagrement' => $cal?->id]) }}"
                                        class="btn btn-primary btn-sm me-2" title="Exporter Fiche de synthèse" target="_blank">
                                        <i class="bi bi-file-earmark-pdf"></i> Fiche de synthèse
                                    </a>
                                    <div class="filter ms-2">
                                        <a class="icon" href="#" data-bs-toggle="dropdown"><i
                                                class="bi bi-three-dots"></i></a>
                                        <ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow">
                                            @foreach ($coupes as $index => $coupe)
                                                @php
                                                    $start = $index * $taille + 1;
                                                    $end = $start + $coupe->count() - 1;
                                                @endphp
                                                <li>
                                                    <a class="dropdown-item btn btn-sm"
                                                        href="{{ route('export.ficheSynthese', [
                                                            'commissionagrement' => $cal->id,
                                                            'offset' => $start - 1,
                                                            'limit' => $coupe->count(),
                                                        ]) }}"
                                                        target="_blank">
                                                        Fiche synthèse {{ $start }} - {{ $end }}
                                                    </a>
                                                </li>
                                            @endforeach
                                    ...
                                    <div class="filter ms-2">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end dropdown-menu-arrow">
                                            ...
                                            @foreach ($chunks as $index => $chunk)
                                                @php
                                                    $start = $index * $step + 1;
                                                    $end = $start + $chunk->count() - 1;
                                                @endphp
                                                <li>
                                                    <a class="dropdown-item btn btn-sm"
                                                        href="{{ route('export.operateurs', [
                                                            'commissionagrement' => $cal->id,
                                                            'offset' => $start - 1,
                                                            'limit' => $chunk->count(),
                                                        ]) }}">
                        ...
                        @if ($cal?->operateurs->isNotEmpty())
                            <div class="table-responsive">
                                <table
                                    ...
                                    <tbody>
                                        <?php $i = 1; ?>
                                        @foreach ($cal?->operateurs as $operateur)
                                            <tr>
                                                ...
                                                @can('operateur-show')
                                                    ...
                                                    <td>
                                                        <span class="d-flex align-items-baseline">
                                                            <a href="{{ route('showAgrement', ['id' => $operateur->id]) }}"
                                                            ...
                                                            <div class="filter">
                                                                ...
                                                                <ul
                                                                    ...
                                                                    @can('operateur-delete')
                                                                        ...
                                                                        {{-- Bouton pour détacher la relation --}}
                                                                        <li>
                                                                            <form
                                                                                action="{{ route('operateurs.detachCommission', [$operateur?->id, $cal?->id]) }}"
                                                                                method="post">
                                                                                @csrf
                                                                                @method('DELETE')
                                                                                <button type="submit"
                                                                                    class="dropdown-item show_confirm_retirer"
                                                                                    title="Détacher de la commission">
                                                                                    <i class="bi bi-x-circle"></i> Détacher
                                                                                </button>
                                                                            </form>
                                                                        </li>
                                                                    @endcan
                        ...
                        @else
                            <div class="alert alert-info">Aucun opérateur pour l'instant !</div>
                        @endif
                    </div>
                </div>
                                        ...
                                        <div class="col-12">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="operateur_name"
                                                    ...
                                                    id="operateur_name" placeholder="Dénomination">
