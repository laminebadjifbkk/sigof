                                            ...
                                            <div class="col-12 col-md-12 col-lg-4 col-sm-12 col-xs-12 col-xxl-4">
                                                ...
                                                <select name="departement" id="module_select_{{ $projetmodule->id }}"
                                                    ...
                                                    <option value="" disabled selected>Choisir
                                                        {{ $projetmodule?->projet?->type_localite }}</option>
                                        ...
                                        <form method="post" action="{{ route('individuelles.update', $individuelle) }}"
                                            ...
                                            <div class="col-12 col-md-12 col-lg-4 col-sm-12 col-xs-12 col-xxl-4">
                                                ...
                                                <select name="departement" id="module_select_{{ $projetmodule->id }}"
                                                    ...
                                                    @php
                                                        // Vérifie si une demande existe déjà pour ce module
                                                        $demandeExistante = $projetmodule->individuelles->firstWhere(
                                                            'users_id',
                                                            Auth::id(),
                                                        );

                                                        // Récupère la localité par défaut (de la demande existante ou une autre valeur par défaut)
                                                        $localiteDefaut = isset($demandeExistante)
                                                            ? $demandeExistante->{strtolower(
                                                                $projetmodule->projet->type_localite,
                                                            )}
                                                            : ''; // Vous pouvez ajuster ceci selon votre logique
                                                    @endphp

                                                    <!-- OPTION PLACEHOLDER avec la localité par défaut -->
                                                    <option value="" disabled
                                                        {{ !$localiteDefaut ? 'selected' : '' }}>
                                                        Choisir {{ $projetmodule?->projet?->type_localite }}
                                                    </option>

                                                    @foreach ($projetmodule->projetlocalites as $projetlocalite)
                                                        <option value="{{ $projetlocalite->localite }}"
                                                            {{ // Vérifie si la demande existe et sélectionne la localité correspondante ou ancienne valeur
                                                                $localiteDefaut == $projetlocalite->localite || old('departement') == $projetlocalite->localite ? 'selected' : '' }}>
                                                            {{ $projetlocalite->localite }}
                                                        </option>
                                                    @endforeach
                                                </select>
