...
@section('title', 'Ajouter responsable ' . $direction?->sigle)
@section('space-work')
            ...
            <div class="col-lg-12">
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="row">
                            <div class="col-sm-12 pt-0">
                                <span class="d-flex mt-0 align-items-baseline"><a
                                        href="{{ route('directions.show', $direction?->id) }}"
                        ...
                        <h5><u><b>Chef</b></u> :
                            {{ $direction?->chef?->user?->civilite . ' ' . $direction?->chef?->user?->firstname . ' ' . $direction?->chef?->user?->name }}
                        </h5>
                        <form method="post" action="{{ url('directionChef', ['$iddirection' => $direction?->id]) }}"
                                ...
                                <div class="col-md-12">
                                    <div class="table-responsive">
                                                ...
                                                @foreach ($employes as $employe)
                                                    <tr>
                                                        <td style="vertical-align:center;">
                                                            <input type="radio" name="employe" value="{{ $employe?->id }}"
                                                                {{ in_array($employe?->id, $employeDirection) ? 'checked' : '' }}
                                                                {{ in_array($employe?->id, $employeDirectionCheck) ? 'disabled' : '' }}
                                                                class="form-check-input @error('employe') is-invalid @enderror">
                                                            @error('employe')
                                                        ...
                                                        <td>
                                                            <span scope="row"><img class="rounded-circle w-20"
                                                                    alt="Profil"
                                        ...
                                        <div class="text-center">
                                            <button type="submit"
                                                class="btn btn-outline-primary btn-sm">Sélectionner</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
