                            ...
                            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
                                ...
                                <h5 class="mb-0 fw-semibold text-info">
                                    <i class="bi bi-list-ul me-1"></i> Formation: <span
                                        class="text-dark">{{ $formation?->intitule }}</span>
                                </h5>
                            </div>
                                ...
                                @foreach ($formation->evaluateurs as $evaluateur)
                                    <div class="border rounded p-3 mb-3 bg-light-subtle">
                                        ...
                                        <div class="row g-3">
                                            ...
                                            <div class="col-md-4">
                                                <label class="form-label">Indemnité (FCFA)<span
                                                        class="text-danger">*</span></label>
                                                <input type="number" step="0.01" min="0"
                                                    name="evaluations[{{ $evaluateur->id }}][indemnite]"
                                                    value="{{ old('evaluations.' . $evaluateur->id . '.indemnite', $evaluateur->pivot->indemnite) }}"
                                                    class="form-control form-control-sm @error('evaluations.' . $evaluateur->id . '.indemnite') is-invalid @enderror"
                                                    placeholder="Ex: 50000">
                                                @error('evaluations.' . $evaluateur->id . '.indemnite')
                                                    <div class="invalid-feedback">{{ $message }}</div>
                                                @enderror
                                            </div>
                                        </div>
