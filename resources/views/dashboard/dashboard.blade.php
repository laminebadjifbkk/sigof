...
@section('content')
    ...
    <div class="kpi-grid">
        ...
        <div class="kpi-card">
            ...
            <div class="kpi-delta">{{ $kpis['total'] > 0 ? round(($kpis['validees'] / $kpis['total']) * 100) : 0 }} % du
                total</div>
        </div>
        <div class="kpi-card">
            <div class="kpi-top"><span class="kpi-label">En attente</span><span class="kpi-dot"
            ...
            <!-- <div class="kpi-delta" style="color:#8A6416;">À traiter</div> -->
            <div class="kpi-delta" style="color:#8A6416;">
                {{ $kpis['total'] > 0 ? round(($kpis['attente'] / $kpis['total']) * 100) : 0 }} % à traiter du total</div>
        </div>
        <div class="kpi-card">
            <div class="kpi-top"><span class="kpi-label">Rejetées</span><span class="kpi-dot"
                    style="background:var(--brick)"></span></div>
            <div class="kpi-num">{{ $kpis['rejete'] }}</div>
            <div class="kpi-delta" style="color:var(--brick);">
                {{ $kpis['total'] > 0 ? round(($kpis['rejete'] / $kpis['total']) * 100) : 0 }} % du total</div>
        </div>
        <!-- <div class="kpi-card">
                <div class="kpi-top"><span class="kpi-label">Traducteurs mobilisés</span><span class="kpi-dot"
                        style="background:var(--brick)"></span></div>
    ...
    <div class="dash-grid">
        ...
        <div class="panel">
            <h3>Répartition par langue de spécialisation</h3>
            <p class="panel-sub">Candidatures reçues - top {{ $languageStats->count() }} langues sur
                {{ \App\Models\LanguesSpecialisation::count() }}</p>
            <div class="bar-chart">
                @php
                ...
                @forelse ($languageStats as $index => $langue)
                    <div class="bar-col">
                        ...
                        <div class="bar"
                            style="height:{{ round(($langue['total'] / $maxLangueTotal) * 100) }}%; background:{{ $colors[$index] ?? 'var(--gray-400)' }};">
                        </div>
        ...
        <div class="panel">
            ...
            <div class="donut-wrap">
                <svg width="140" height="140" viewBox="0 0 42 42">
                    <circle cx="21" cy="21" r="15.9" fill="transparent" stroke="var(--gray-200)"
                        stroke-width="6" />
                    <circle cx="21" cy="21" r="15.9" fill="transparent" stroke="var(--green)"
                        stroke-width="6" stroke-dasharray="{{ $valideesLen }} {{ 100 - $valideesLen }}"
                        stroke-dashoffset="25" transform="rotate(-90 21 21)" />
                    <circle cx="21" cy="21" r="15.9" fill="transparent" stroke="var(--gold)"
                        stroke-width="6" stroke-dasharray="{{ $attenteLen }} {{ 100 - $attenteLen }}"
                        stroke-dashoffset="{{ 25 - $valideesLen }}" transform="rotate(-90 21 21)" />
                    <circle cx="21" cy="21" r="15.9" fill="transparent" stroke="var(--brick)"
                        stroke-width="6" stroke-dasharray="{{ $rejeteesLen }} {{ 100 - $rejeteesLen }}"
                        stroke-dashoffset="{{ 25 - $valideesLen - $attenteLen }}" transform="rotate(-90 21 21)" />
                </svg>
                <ul class="donut-legend">
                    <li><span class="sw" style="background:var(--green)"></span>Validées -
                        {{ $statutStats['validees_pct'] }} %</li>
                    <li><span class="sw" style="background:var(--gold)"></span>En attente -
                        {{ $statutStats['attente_pct'] }} %</li>
                    <li><span class="sw" style="background:var(--brick)"></span>Rejetées -
                        {{ $statutStats['rejetees_pct'] }} %</li>
                </ul>
            </div>
        </div>
    </div>
